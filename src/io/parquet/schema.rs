...
use parquet::schema::types::{ColumnDescriptor, SchemaDescriptor, Type, TypePtr};
use parquet::{
...
use crate::io::ipc::{
    ...
    gen::Message::root_as_message,
    write::common::{IpcDataGenerator, IpcWriteOptions},
};

use super::ARROW_SCHEMA_META_KEY;
...
fn get_arrow_schema_from_metadata(encoded_meta: &str) -> Result<Schema> {
    ...
    match decoded {
        Ok(bytes) => {
            ...
            match root_as_message(slice) {
                Ok(message) => {
                    message
                        .header_as_schema()
                        .map(fb_to_schema)
                        .ok_or(ArrowError::ExternalFormat(
                            "the message is not Arrow Schema".to_string(),
                        ))
                }
                Err(err) => {
...
pub(crate) fn add_encoded_arrow_schema_to_metadata(
    ...
    let data_pagesize_limit = props.data_pagesize_limit();
    let dictionary_pagesize_limit = props.dictionary_pagesize_limit();
    let write_batch_size = props.write_batch_size();
    let max_row_group_size = props.max_row_group_size();
    let writer_version = props.writer_version();
    let column_properties = props.column_properties();
    ...
    WriterProperties::builder()
        ...
        .set_dictionary_pagesize_limit(dictionary_pagesize_limit)
        .set_write_batch_size(write_batch_size)
        .set_max_row_group_size(max_row_group_size)
        .set_writer_version(writer_version)
        .set_encoding(props.encoding())
        .build()
}
 */
...
fn arrow_to_parquet_type(field: &Field) -> Result<Type> {
    ...
    match field.data_type() {
        ...
        DataType::Time64(_) => Type::primitive_type_builder(name, PhysicalType::INT64)
            ...
            .with_repetition(repetition)
            .build(),
        DataType::Duration(_) => Err(ParquetError::General(
        ...
        DataType::Dictionary(_, ref value) => {
            ...
            return arrow_to_parquet_type(&dict_field);
        }
    }
    .map_err(|x| x.into())
}
...
impl ParquetTypeConverter<'_> {
    ...
    fn from_int32(&self) -> Result<DataType> {
        match self.schema.get_basic_info().logical_type() {
            ...
            other => Err(ArrowError::ExternalFormat(format!(
    ...
    fn from_int64(&self) -> Result<DataType> {
        match self.schema.get_basic_info().logical_type() {
            ...
            LogicalType::TIMESTAMP_MILLIS => Ok(DataType::Timestamp(TimeUnit::Millisecond, None)),
            LogicalType::TIMESTAMP_MICROS => Ok(DataType::Timestamp(TimeUnit::Microsecond, None)),
            LogicalType::DECIMAL => Ok(self.to_decimal()),
            other => Err(ArrowError::ExternalFormat(format!(
    ...
    fn to_struct(&self) -> Result<Option<DataType>> {
        match self.schema {
            Type::PrimitiveType { .. } => Err(ArrowError::ExternalFormat(format!(
    ...
    fn to_list(&self) -> Result<Option<DataType>> {
        match self.schema {
            ...
            Type::GroupType {
                ...
                let item_type = match list_item.as_ref() {
                    ...
                    Type::GroupType {
                            ...
                            && list_item.name() != format!("{}_tuple", self.schema.name())
                        {
                            ...
                            let nested_item_converter = self.clone_with_schema(nested_item);
                ...
                item_type.map(|opt| {
                    opt.map(|dt| {
                        DataType::List(Box::new(Field::new(list_item_name, dt, item_is_optional)))
                    })
                })
            }
            _ => Err(ArrowError::ExternalFormat(
