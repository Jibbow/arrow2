...
use parquet::schema::types::{ColumnDescriptor, ColumnPath, SchemaDescPtr, Type, TypePtr};
use parquet::schema::visitor::TypeVisitor;
use parquet::{
...
use parquet::{
    errors::ParquetError,
    file::reader::{FilePageIterator, FileReader},
};

use crate::datatypes::{Field, Schema};
use crate::error::{ArrowError, Result};
use crate::{compute::cast::can_cast_types, datatypes::DataType};

use super::read::{
    converter::{BinaryArrayConverter, BinaryConverter, DecimalArrayConverter, DecimalConverter},
    ArrayReader, BooleanArrayReader, PrimitiveArrayReader, StructArrayReader,
    VariableSizeArrayReader,
};
use super::{
    read::converter::{Utf8ArrayConverter, Utf8Converter},
    read::DeprecatedInt96Timestamp,
    schema::parquet_to_arrow_field,
};
    ...
    T: IntoIterator<Item = usize>,
{
    ...
    if leaves.is_empty() {
        return Err(ArrowError::ExternalFormat(format!(
            "Can't build array reader without columns!"
        )));
    }
...
pub struct ListArrayReader<OffsetSize: Offset> {
...
impl<OffsetSize: Offset> ListArrayReader<OffsetSize> {
...
fn remove_indices(
    ...
    match item_type {
        DataType::UInt8 => remove_primitive_array_indices!(arr, ArrowUInt8Type, indices),
        DataType::UInt16 => {
        ...
        DataType::Int8 => remove_primitive_array_indices!(arr, ArrowInt8Type, indices),
        DataType::Int16 => remove_primitive_array_indices!(arr, ArrowInt16Type, indices),
        DataType::Int32 => remove_primitive_array_indices!(arr, ArrowInt32Type, indices),
        DataType::Int64 => remove_primitive_array_indices!(arr, ArrowInt64Type, indices),
        DataType::Float32 => {
        ...
        DataType::Duration(ArrowTimeUnit::Millisecond) => {
            remove_primitive_array_indices!(arr, ArrowDurationMillisecondType, indices)
        }
        DataType::Duration(ArrowTimeUnit::Microsecond) => {
        ...
        DataType::FixedSizeBinary(size) => remove_fixed_size_binary_array_indices!(
            arr,
...
impl<OffsetSize: Offset> ArrayReader for ListArrayReader<OffsetSize> {
    fn as_any(&self) -> &dyn Any {
        self
    }
    ...
    for ArrayReaderBuilder
{
    ...
    fn visit_primitive(
    ...
    ) -> std::result::Result<Option<Box<dyn ArrayReader>>, ParquetError> {
        if self.is_included(cur_type.as_ref()) {
            ...
            let reader = self
                .build_for_primitive_type_inner(cur_type.clone(), &new_context)
                .map_err::<ParquetError, _>(|x| x.into())?;
    ...
    fn visit_struct(
        ...
        if let Some(reader) = self
            .build_for_struct_type_inner(&cur_type, &new_context)
            .map_err::<ParquetError, _>(|x| x.into())?
        {
            if cur_type.get_basic_info().has_repetition()
...
fn cast_gate(
    physical_parquet: &PhysicalType,
    base_data_type: &DataType,
    data_type: DataType,
) -> Result<DataType> {
    if can_cast_types(&base_data_type, &data_type) {
        Ok(data_type)
    } else {
        Err(ArrowError::NotYetImplemented(format!(
            "The parser was requested to read parquets' physical type {:?} \
             into arrows' logical type {:?} but \
             arrow does not support casting from {:?} to {:?}",
            physical_parquet, data_type, base_data_type, data_type
        )))
    }
}
...
fn make_reader(
    physical_parquet: &PhysicalType,
    logical_parquet: &LogicalType,
    data_type: DataType,
    pages: Box<dyn PageIterator>,
    column_desc: Arc<ColumnDescriptor>,
) -> Result<Box<dyn ArrayReader>> {
    match (physical_parquet, logical_parquet, &data_type) {
        // booleans are booleans
        (PhysicalType::BOOLEAN, _, DataType::Boolean) => {
            BooleanArrayReader::new(pages, column_desc, data_type)
                .map(Box::new)
                .map(|x| x as Box<dyn ArrayReader>)
        }
        // All Int64 are read as primitives
        (PhysicalType::INT32, _, _) => {
            let base_data_type = DataType::Int32;
            let cast_data_type = cast_gate(physical_parquet, &base_data_type, data_type)?;
        ...
        (PhysicalType::INT64, _, _) => {
            let base_data_type = DataType::Int64;
            let cast_data_type = cast_gate(physical_parquet, &base_data_type, data_type)?;
        ...
        // Float are read as float
        (PhysicalType::FLOAT, _, _) => {
            let base_data_type = DataType::Float32;
            let cast_data_type = cast_gate(physical_parquet, &base_data_type, data_type)?;
        ...
        (PhysicalType::DOUBLE, _, DataType::Float64) => {
            let base_data_type = DataType::Float64;
            let cast_data_type = cast_gate(physical_parquet, &base_data_type, data_type)?;
        ...
        (PhysicalType::INT96, _, DataType::Timestamp(_, _)) => {
            DeprecatedInt96Timestamp::new(pages, column_desc, data_type)
                .map(Box::new)
                .map(|x| x as Box<dyn ArrayReader>)
        }
        (PhysicalType::BYTE_ARRAY, _, DataType::Utf8) => {
            VariableSizeArrayReader::<ByteArrayType, _>::new(
                pages,
                column_desc,
                Utf8Converter::<i32>::new(Utf8ArrayConverter {}),
                data_type,
            )
            .map(Box::new)
            .map(|x| x as Box<dyn ArrayReader>)
        }
        (PhysicalType::BYTE_ARRAY, _, DataType::LargeUtf8) => {
            VariableSizeArrayReader::<ByteArrayType, _>::new(
                pages,
                column_desc,
                Utf8Converter::<i64>::new(Utf8ArrayConverter {}),
                data_type,
            )
            .map(Box::new)
            .map(|x| x as Box<dyn ArrayReader>)
        ...
        (PhysicalType::BYTE_ARRAY, _, DataType::Binary) => {
            VariableSizeArrayReader::<ByteArrayType, _>::new(
                pages,
                column_desc,
                BinaryConverter::<i32>::new(BinaryArrayConverter {}),
                data_type,
            )
            .map(Box::new)
            .map(|x| x as Box<dyn ArrayReader>)
        }
        (PhysicalType::BYTE_ARRAY, _, DataType::LargeBinary) => {
            VariableSizeArrayReader::<ByteArrayType, _>::new(
                pages,
                column_desc,
                BinaryConverter::<i64>::new(BinaryArrayConverter {}),
                data_type,
            )
            .map(Box::new)
            .map(|x| x as Box<dyn ArrayReader>)
        }
        (PhysicalType::FIXED_LEN_BYTE_ARRAY, LogicalType::DECIMAL, DataType::Decimal(_, _)) => {
            let converter = DecimalConverter::new(DecimalArrayConverter::new(
                column_desc.self_type().get_precision(),
                column_desc.self_type().get_scale(),
            ));
            VariableSizeArrayReader::<FixedLenByteArrayType, _>::new(
                pages,
                column_desc,
                converter,
                data_type,
...
impl<'a> ArrayReaderBuilder {
    ...
    fn build_array_reader(&mut self) -> Result<Box<dyn ArrayReader>> {
        ...
        self.visit_struct(self.root_schema.clone(), &context)
            .and_then(|reader_opt| {
                reader_opt
                    .ok_or_else(|| ParquetError::General(format!("Failed to build array reader!")))
            })
            .map_err(|x| x.into())
    }
    ...
    /// Creates primitive array reader for a primitive type.
    fn build_for_primitive_type_inner(
        ...
        let data_type = match self.get_arrow_field(&cur_type, context) {
            Some(f) => f.data_type().clone(),
            _ => parquet_to_arrow_field(column_desc.as_ref())?
                .data_type()
                .clone(),
        };
        ...
        make_reader(
            &cur_type.get_physical_type(),
            &cur_type.get_basic_info().logical_type(),
            data_type,
            pages,
            column_desc,
        )
    ...
    fn build_for_struct_type_inner(
        ...
        if !fields.is_empty() {
            let arrow_type = DataType::Struct(fields);
...
mod tests {
    ...
    use crate::data_type::{ByteArray, ParquetType, Int32Type, Int64Type};
    ...
    use arrow::datatypes::{
        ArrowPrimitiveType, ParquetType as DataType, Date32Type as ArrowDate32, Field,
    ...
    fn make_column_chunks<T: ParquetType>(
        column_desc: ColumnDescPtr,
        encoding: Encoding,
        num_levels: usize,
        min_value: T::T,
