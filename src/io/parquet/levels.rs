...
use crate::array::{Array, ListArray, Offset, StructArray};
...
fn get_array_offsets_and_masks(array: &dyn Array) -> (Vec<i64>, Vec<bool>) {
    match array.data_type() {
        ...
        | DataType::Decimal(_, _) => {
            let array_mask = match array.validity() {
        ...
        DataType::FixedSizeBinary(_) | DataType::FixedSizeList(_, _) | DataType::Union(_) => {
            unimplemented!("Getting offsets not yet implemented")
        }
    }
}

fn get_list<O: Offset>(
    list_level: LevelInfo,
    list_field: &Field,
    array: &ListArray<O>,
) -> Vec<LevelInfo> {
    // Construct the child array of the list, and get its offset + mask
    let child_array = array.values();
    let (child_offsets, child_mask) = get_array_offsets_and_masks(child_array.as_ref());
    ...
    match child_array.data_type() {
        ...
        DataType::List(_) | DataType::LargeList(_) | DataType::Struct(_) => {
            list_level.calculate_array_levels(child_array.as_ref(), list_field)
        }
...
impl LevelInfo {
    ...
    pub(crate) fn calculate_array_levels(&self, array: &dyn Array, field: &Field) -> Vec<Self> {
        let (array_offsets, array_mask) = get_array_offsets_and_masks(array);
        match array.data_type() {
            ...
            DataType::List(list_field) => {
                ...
                let array = array.as_any().downcast_ref::<ListArray<i32>>().unwrap();
                get_list(list_level, list_field, array)
            }
            ...
            DataType::Struct(_) => {
                ...
                array
                    ...
                    .for_each(|(child_array, child_field)| {
                        let mut levels =
                            struct_level.calculate_array_levels(child_array.as_ref(), child_field);
                        ...
                        (start..end).for_each(|parent_index| {
                            ...
                            if child_len == 0 {
                                ...
                                (child_start..child_end).for_each(|child_index| {
                                    let rep =
                                        match (parent_index == start, child_index == child_start) {
                    ...
                    .for_each(|(parent_index, def)| {
                        ...
                        match (parent_mask, child_len) {
                            (true, 0) => {
                                ...
                                definition.push(if child_mask { l2 } else { self.max_definition });
                                repetition.push(0);
                                merged_array_mask.push(child_mask);
                            }
                            (false, 0) => {
                            ...
                            (true, _) => {
                                (child_from..child_to).for_each(|child_index| {
                                    ...
                                    repetition.push(if child_index == child_from { 0 } else { 1 });
                                    merged_array_mask.push(child_mask);
                                });
                            }
                            (false, _) => {
...
mod tests {
    ...
    use crate::{
        array::{Array, ListArray, ListPrimitive, Primitive},
        datatypes::Schema,
    };
    ...
    fn test_calculate_array_levels_twitter_example() {
        ...
        let levels =
            parent_levels.calculate_child_levels(array_offsets.clone(), array_mask, true, false);
        //
    ...
    fn test_calculate_array_levels_1() {
        ...
        let levels =
            parent_levels.calculate_child_levels(array_offsets.clone(), array_mask, true, true);
    ...
    fn test_calculate_nested_struct_levels() {
        ...
        let b_levels = a_levels.calculate_child_levels(b_offsets.clone(), b_mask, false, true);
    ...
    fn list_single_column() {
        ...
        let list = a.to(ListArray::<i32>::default_datatype(DataType::Int32));

        debug_assert_eq!(list.null_count(), 1);

        let schema = Schema::new(vec![Field::new("item", list.data_type().clone(), true)]);
        ...
        batch
            ...
            .for_each(|(array, field)| {
                let mut array_levels = batch_level.calculate_array_levels(array.as_ref(), field);
